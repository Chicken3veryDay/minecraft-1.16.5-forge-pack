...
param(
    ...
    [string]$PackName = 'Forge 1.20.1 ProjectE Chaos Pack',
...
function Assert-GitHubCli {
    if (-not (Get-Command gh -ErrorAction SilentlyContinue)) {
        throw "GitHub CLI ('gh') is required for -Upload or -VerifyHostedRelease."
    }
}
...
function Assert-HostedAssetMatchesLocal {
    ...
    if (-not $Assets.ContainsKey($Name)) {
        throw "Hosted release is missing asset '$Name'."
    }
    ...
    if ([string]::IsNullOrWhiteSpace($digest)) {
        Write-Host "Hosted '$Name' has no digest; size verified only."
        return
    }

    $hostedHash = $digest.Replace('sha256:', '').ToLowerInvariant()
    if ($hostedHash -ne $Sha256) {
        throw "Hosted '$Name' SHA-256 mismatch. Expected $Sha256, got $hostedHash."
    }
}

function Update-ManifestAssetArchiveName {
    param([string]$Name)

    $manifest = Get-Content -LiteralPath $ManifestPath -Raw | ConvertFrom-Json
    $manifest.assetArchive.name = $Name
    $assetUrl = "https://github.com/$Owner/$Repo/releases/download/$Tag/$Name"
    if ($manifest.assetArchive.PSObject.Properties.Name -contains 'url') {
        $manifest.assetArchive.url = $assetUrl
    }
    else {
        $manifest.assetArchive | Add-Member -NotePropertyName 'url' -NotePropertyValue $assetUrl
    }
    $manifest | ConvertTo-Json -Depth 12 | Set-Content -LiteralPath $ManifestPath -Encoding UTF8
}

Write-Step "Refreshing manifest and pack summary"
& (Join-Path $PackRoot 'tools\Build-ForgePackAssets.ps1')
& (Join-Path $PackRoot 'tools\Update-ModList.ps1')
...
Update-ManifestAssetArchiveName -Name $ReleaseAssetArchiveName
...
if (-not $SkipMinimalZip) {
    ...
    $manifest = Get-Content -LiteralPath $ManifestPath -Raw | ConvertFrom-Json
    $forgeInstallerName = "forge-$($manifest.minecraft)-$($manifest.forgeVersion)-installer.jar"
    $forgeInstallerPath = Join-Path $PackRoot "pack-sources\Server\$forgeInstallerName"
    if (-not (Test-Path -LiteralPath $forgeInstallerPath)) {
        throw "Missing Forge installer for minimal zip: $forgeInstallerPath"
    }
    ...
    Invoke-CompressArchiveWithRetry -LiteralPath @(
        ...
        (Join-Path $PackRoot 'Install-Forge-Server.sh'),
        (Join-Path $PackRoot 'Install-Fabric-Server.sh'),
...
if ($VerifyHostedRelease) {
    ...
    Assert-HostedAssetMatchesLocal -Assets $assets -Name $ReleaseAssetArchiveName -Path $ReleaseAssetArchivePath -Sha256 $assetArchiveHash
    if (-not $SkipMinimalZip) {
        Assert-HostedAssetMatchesLocal -Assets $assets -Name $ReleaseMinimalZipName -Path $ReleaseMinimalZipPath -Sha256 $minimalHash
    }
}
